...
class Phoneme:
    ...
    def set_poa(
        self, row
    ):  # gives a number where consonants at the front of the mouth come first
        self.poa = 0
        ...
        if row["advancedTongueRoot"] == "+":
            vowel_pos -= 0.1
        if (
            row["SegmentClass"] == "vowel" or self.moa == "vocalic"
...
class Language:
    ...
    def get_vector(self, phoneme_list, phoneme_to_id):
        ...
        for phoneme in self.phonemes:
            out_vector[phoneme_to_id[phoneme.ipa]] = 1.0  # one hot encoding
